...
#define t_assert_floats(data, ...) \
	do { \
		static const float expected[] = {__VA_ARGS__}; \
		const bool got_expected = memcmp(data, expected, sizeof(expected)) == 0; \
		if (!got_expected) { \
			return "assertion failed: " #data " did not contain {" #__VA_ARGS__ "} as expected (" t_lineno ")"; \
		} \
	} while (0);
